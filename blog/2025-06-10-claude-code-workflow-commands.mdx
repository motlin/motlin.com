...
title: "Claude Code Secrets: The Perfect Development Loop"
authors: [craig]
tags: [claude-code, developer-productivity, workflow]
date: 2025-06-10
---
...
I've written two slash commands for Claude Code that form the core of my development workflow: `/user:todo` implements one task and `/user:commit` commits it.
...
- **Global commands**: Place them in `$HOME/.claude/commands/example.md` and invoke with `/user:example`
- **Project commands**: Place them in `./.claude/commands/` and invoke with `/project:example`

## The Development Loop

When building larger features, I work through a task list in `.llm/todo.md`:

1. Run `/user:todo` to implement the next task
2. Test and debug manually
3. Run `/user:commit` to commit the changes
...
After implementing and testing a task, I use `/user:commit` to create a commit:
